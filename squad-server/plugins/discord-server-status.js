...
export default class DiscordServerStatus extends DiscordBaseMessageUpdater {
  ...
  async updateStatus() {
    ...
    let queue = '';
    if (this.server.publicQueue + this.server.reserveQueue > 0)
      queue = `(+${this.server.publicQueue + this.server.reserveQueue})`;

    const layerName = (await this.server.rcon.getCurrentMap()).layer

    await this.options.discordClient.user.setActivity(
      `${this.server.a2sPlayerCount}/${this.server.publicSlots}${queue} ${
        layerName || ' '
      }`,
      { type: 4 }
    );
